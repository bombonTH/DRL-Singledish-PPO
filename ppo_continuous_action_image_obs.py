...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.bore_encoder = nn.Sequential(
            # nn.Conv2d(in_channels=4, out_channels=32, kernel_size=3, padding=1, stride=1, groups=1), # 32x64x64
    ...
    def get_value(self, x):
        feature = torch.cat((self.encoder(x[:,:3]), self.bore_encoder(x[:,3:4]), x[:,0,:7,0]), 1)
        latent_action = self.actor(feature)
        return self.critic(latent_action)

    def get_action_and_value(self, x, action=None):
        self.update_count += 1
        feature = torch.cat((self.encoder(x[:,:3]), self.bore_encoder(x[:,3:4]), x[:,0,:7,0]), 1)
        latent_action = self.actor(feature)
        action_mean = self.actor_mean(latent_action)
        ...
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(latent_action)
